...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Lift merged 2D pose + DA3 depth into 3D pose for every participant "
        "recording and every reference video. Runs with matching merged pose CSVs and depth "
        "predictions already present; others are skipped (see estimate_monocular_depth.py / "
        "merge_poses.py)."
    )
    parser.add_argument(
        "--project_folder",
    ...
    parser.add_argument("--shoulder_width_m", type=float, default=0.40)
...
def main(args):
    ...
    lift_script = script_folder / "estimate_3d_pose.py"

    common_args = [
        "--shoulder_width_m", str(args.shoulder_width_m),
        "--min_likelihood", str(args.min_likelihood),
    ]
    if args.focal_length_px is not None:
        common_args += ["--focal_length_px", str(args.focal_length_px)]
    if args.save_vis:
    ...
    if not args.skip_participants:
        ...
        for school, participant_id in participants:
            ...
            command = [
                sys.executable, str(lift_script),
