"""Per-frame kinematic feature time series for each reference clip, in 3D.

3D counterpart of analysis_2d/reference_timeseries.py. For every reference clip, plots
each of the underlying kinematic signals (features.SIGNAL_NAMES -- the same ones
aggregated into the *_mean/*_std columns used everywhere else in this analysis, now
computed in 3D in a per-frame body-centered coordinate frame) across the clip's frames,
one subplot per signal, and saves it into that reference's output/knn/<clip_id>/ folder
alongside the existing t-SNE/KNN outputs.
