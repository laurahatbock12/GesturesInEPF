...
def load_full_body_predictions(run, process_reference_videos):
...
def main(args):
    ...
    mode = args.model
    force = args.force
    process_reference_videos = args.process_reference_videos

    if process_reference_videos:
        video_folders = [i for i in Path(project_folder).iterdir() if i.is_dir() and i.name.startswith("EPF_PL_")]
        runs = [i for folder in video_folders for i in folder.iterdir() if i.name.endswith(".mp4")]
        if not len(runs):
    ...
    for run in runs:
        try:
            if process_reference_videos:
                output_dir = run.parent / "pose_estimation" / f"{mode}_predictions"
            ...
            full_body_predictions = load_full_body_predictions(run, process_reference_videos) if mode == "hand" else None
