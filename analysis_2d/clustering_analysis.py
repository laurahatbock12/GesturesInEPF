...
Pipeline:
  1. Load all merged-prediction clips (participants + references) via dataloader.py.
  ...
  3. Slide a window (mean reference duration, 50% overlap) over every participant clip and
     extract the 20 aggregate kinematic features (features.py) from each window; extract the
