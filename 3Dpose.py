...
def process_frame(inferencer, frame_rgb):
    ...
    # Manually draw keypoints onto the original frame
    ...
    if results and 'predictions' in results:
        for pred in results['predictions'][0]:
            ...
            for kpt, s in zip(keypoints, scores):
                # 3D keypoints have x, y, z — only use x, y for drawing
                x, y = int(kpt[0]), int(kpt[1])
                if s >= POSE_THRESHOLD:
                    cv2.circle(vis_img, (x, y), 4, (0, 255, 0), -1)

    results['visualization'] = [vis_img]
    return results
    ...
    if results and 'predictions' in results:
        for pred in results['predictions'][0]:
            ...
            for kpt, s in zip(keypoints, scores):
                ...
                coords = [float(c) for c in kpt]
                coords.append(float(s))
                person_data["keypoints"].append(coords)
...
def process_video():
    ...
    out_video  = os.path.join(OUTPUT_DIR, f"{video_name}_3dbody.mp4")
    out_json   = os.path.join(OUTPUT_DIR, f"{video_name}_keypoints_3dbody.json")
    out_csv    = os.path.join(OUTPUT_DIR, f"{video_name}_keypoints_3dbody.csv")
